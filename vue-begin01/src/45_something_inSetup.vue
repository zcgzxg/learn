...
<script>
import{ref, reactive, watch, watchEffect, computed, onBeforeMount, onMounted, onBeforeUpdate, onUpdated} from 'vue'
export default {
    data(){
        return{
    ...
    setup(){
        ...
        const user = reactive({
            name:'user',
            age:18,
            reverseMsg:computed(() => {// 返回一个带有
                return msgReactive.value.split('').reverse().join('')
            })
        })
